...
try:
    ...
    with st.sidebar:
        ...
        # Esto asegura que se ordenen cronológicamente y no alfabéticamente
        ...
        if modo_analisis == "📊 Análisis Global":
            ...
            meses_sel = st.multiselect(
                ...
                format_func=formato_fecha_linda  # <--- AQUÍ ESTÁ LA MAGIA
            )
        else:
            ...
            with col1:
                periodo_a = st.selectbox(
                    ...
                    options=fechas_unicas, 
                    index=idx_a,
                    format_func=formato_fecha_linda # <--- MAGIA
                )
            with col2:
    ...
    # Filtramos directamente por los objetos fecha (no hace falta convertirlos a string)
    ...
    if modo_analisis == "📊 Análisis Global":
        ...
        st.subheader(f"Resumen: {', '.join(nombres_meses)}")
    ...
    # MODO 2: COMPARATIVA
        ...
        st.subheader(f"🆚 Comparativa: {nombre_a} vs {nombre_b}")
